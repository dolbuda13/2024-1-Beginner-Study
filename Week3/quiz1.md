...
- 답: 1. 새로운 branch의 모든 commit이 main branch에 그대로 병합됨 2. 새로운 branch의 모든 commit이 main branch의 새로운 commit하나로 병합(이때, 새로운 commit은 둘 모두를 부모로 함) 3. 새로운 branch의 모든 commit의 base를 재설정, 모두 새로운 commit으로 변경
...
- 답: commit id, 작업 위치 변경, readme 문서화하여 추가
...
- 답: revert 선택. reset은 commit을 삭제하기 때문에 위험, commit을 공유하는 다른 branch에 영향 가능
